...
ProductionTableModel::ProductionTableModel(plan::Subfactory** subfactory, data::Library* db, QObject *parent) :
...
QVariant ProductionTableModel::data(const QModelIndex &index, int role) const {
    const auto &product_line = (*subfactory_)->productLineAt(index.row());
    ...
    if (role == Qt::ItemDataRole::DisplayRole) {
        if (column == Column::Recipe) {
			...
			return QString("%1%").arg(product_line->clockSpeed());
        ...
        if (column == Column::Checked) {
            return product_line->isMarkedComplete() ? Qt::CheckState::Checked : Qt::CheckState::Unchecked;
        ...
        if (column == Column::Product) {
            ...
            // in question produces two and then return whichever is not the targeted targetItem
...
bool ProductionTableModel::setData(const QModelIndex &index, const QVariant &value, int role) {
    const auto column = static_cast<Column> (index.column());
    auto product_line = (*subfactory_)->productLineAt(index.row());
    bool success = false;
    ...
    if (role == Qt::ItemDataRole::CheckStateRole) {
		...
		if (column == Column::Percentage) {
			...
			product_line->setClockSpeed(value.value<float>());
			success = true;
...
bool ProductionTableModel::insertRows(int startRow, const QModelIndex &parent, const plan::ProductLine& product_line) {
    beginInsertRows(parent, startRow, startRow);

    (*subfactory_)->addProductLine(product_line);
...
bool ProductionTableModel::removeRows(int startRow, int count, const QModelIndex &parent) {
    if (count == 0 || !(*subfactory_)->hasProductLines()) {
        return false;
    }

    const int end_row = startRow + count - 1;
    beginRemoveRows(parent, startRow, end_row);
    (*subfactory_)->removeProductLines(startRow, end_row);
    endRemoveRows();
    refreshModel();
    return true;
}
