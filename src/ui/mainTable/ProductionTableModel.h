...
class ProductionTableModel final : public QAbstractTableModel {
    Q_OBJECT
  public:
    explicit ProductionTableModel(plan::Subfactory** subfactory, data::Library* db, QObject* parent = nullptr);

    [[nodiscard]] int rowCount(const QModelIndex &parent) const final { return (*subfactory_)->numProductLines(); }
    ...
    bool insertRows(int startRow, const QModelIndex &parent, const plan::ProductLine& product_line);
