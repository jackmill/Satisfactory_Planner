...
IconListDelegate::IconListDelegate(std::shared_ptr<plan::Subfactory> subfactory, QWidget* parent) :
...
void IconListDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
	...
	auto* ingredients_widget = new IngredientIconList(line.ingredients());
