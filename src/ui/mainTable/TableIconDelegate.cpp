...
TableIconDelegate::TableIconDelegate(std::shared_ptr<plan::Subfactory> subfactory, QWidget* parent) :
...
void TableIconDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
	...
	if (column == ProductionTableModel::Column::Product) {
		button_item = std::make_unique<data::Item>(*line.target());
		...
		auto* icon = new TableIcon(QString::fromStdString(line.recipe().machine().name()),
		                           line.multiplier(),
